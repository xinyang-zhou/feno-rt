"""Run the counterbalanced formal FCFS/cache-aware scheduler matrix."""
...
RUNNER = PROJECT_ROOT / "benchmarks/benchmark_scheduler_ab.py"
...
from benchmarks.benchmark_scheduler_ab import load_checked_manifest  # noqa: E402
from benchmarks.summarize_scheduler_ab import (  # noqa: E402
    summarize_directory,
    write_json_atomic,
)
...
def build_plan(config: Mapping[str, Any]) -> List[Dict[str, Any]]:
    policies = [str(value) for value in config["scheduler"]["policies"]]
    if set(policies) != {"fcfs", "cache_aware"}:
        raise ValueError("scheduler A/B matrix requires exactly fcfs and cache_aware")
    scenarios = [str(item["name"]) for item in config["workloads"]["scenarios"]]
    repeats = int(config["measurement"]["independent_runs"])
    plan: List[Dict[str, Any]] = []
    sequence = 1
    for repeat_index in range(1, repeats + 1):
        scenario_order = scenarios if repeat_index % 2 else list(reversed(scenarios))
        policy_order = policies if repeat_index % 2 else list(reversed(policies))
        for scenario in scenario_order:
            pair_id = f"{scenario}_run{repeat_index}"
            for pair_position, policy in enumerate(policy_order, start=1):
...
def child_command(
    ...
    return [
        ...
        "--output",
        str((output_dir / str(entry["result_file"])).resolve()),
    ]
...
def validate_preconditions(
    ...
    for scenario in config["workloads"]["scenarios"]:
        load_checked_manifest(config, str(scenario["name"]))
...
def existing_result_status(
    ...
    result_path = output_dir / str(entry["result_file"])
    ...
    matches = (
        result["execution"]["policy"] == entry["policy"]
        and result["workload"]["name"] == entry["scenario"]
...
def run_matrix(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    try:
        for entry in session["execution_order"]:
            ...
            print(
                f"[{entry['sequence']:02d}/{len(session['execution_order'])}] "
                f"{entry['policy']}, {entry['scenario']}, repeat {entry['repeat_index']}",
                flush=True,
            )
            try:
    ...
    result_paths = sorted(output_dir.glob("scheduler_*_run*.json"))
    if result_paths:
        try:
            summary = summarize_directory(
                ...
                output_dir / "reuse_throughput.svg",
            )
        except (OSError, ValueError, json.JSONDecodeError) as error:
